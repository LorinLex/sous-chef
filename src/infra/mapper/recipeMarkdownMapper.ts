...
import {
  ...
  RecipeStepProps,
} from "../../domain/recipe/valueObject/types"
...
export class RecipeMarkdownMapper {
  ...
  private parseIngredient({
    ...
    if (
      !rawIngredient.baseNutritionSnapshot ||
      typeof rawIngredient.baseNutritionSnapshot !== "object"
    )
      throw new Error("Ingredient has no nutrition!")

    // Better to another method?
    const rawBaseNutritionSnapshot =
      rawIngredient.baseNutritionSnapshot as Record<string, unknown>
    if (
  ...
  toPrimitives({
    ...
    if (!frontMatter.ingredients) throw new Error("Ingredients are missing!")
    ...
    // steps
    if (!frontMatter.steps) throw new Error("Ingredients are missing!")
    ...
    const time = this.parseTime({
      raw: frontMatter.time as Record<string, unknown>,
    })
