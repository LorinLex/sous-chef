...
export class Recipe {
  ...
  private static ensureInvariants(
    props: CreateRecipeProps | PrimitiveRecipeProps,
  ) {
    if (!props.name.trim()) throw new Error("Recipe name must be non empty!")
  ...
  public static create(props: CreateRecipeProps): Recipe {
    ...
    const ingredientsVO = props.ingredients.map((ingredient) =>
      RecipeIngredient.create(ingredient),
    )
    const stepsVO = props.steps.map((step) => RecipeStep.create(step))
    ...
    return new Recipe({
      ...
      recipeType: parseStringEnum(RecipeType, props.recipeType),
    })
  }
